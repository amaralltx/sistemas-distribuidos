...
TAG = "\033[93m[Consumidor C2 - Todas as Categorias]\033[0m"
RABBITMQ_HOST = 'localhost'
EXCHANGE = 'Promoções'
EXCHANGE_TYPE = 'topic'
NOME_FILA = 'fila_c2'
ROUTING_KEYS = ['promocao.categoria.*']  
...
def main():
    ...
    def callback(ch, method, properties, body):
        # 1. Extrai o envelope da mensagem (Headers)
        headers = properties.headers or {}
        assinatura = headers.get('Signature')
        remetente = headers.get('remetente')

        # 2. Verifica se a mensagem possui assinatura e remetente
    ...
    print(f"{TAG} Aguardando promoções de todas as categorias. Pressione Ctrl+C para encerrar.")
    try:
        channel.start_consuming()
    except KeyboardInterrupt:
        print(f"\n{TAG} Encerrando consumidor C2...")
